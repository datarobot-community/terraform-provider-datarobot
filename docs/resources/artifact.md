...
- `dir_hash` (String) SHA-256 fingerprint of uploadable files under `dir` after `.drignore` / system excludes. Used to detect changes and skip the sync when unchanged. Files covered by a system exclude, including the sync state directory, are never part of this hash. When the directory differs from state it plans as known after apply, because the sync may add or remove files under `dir`; the value recorded is the digest of the directory once the sync is done.
