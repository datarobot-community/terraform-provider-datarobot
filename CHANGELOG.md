...
- `datarobot_artifact` `source.dir` is now synchronized with the catalog using the same three-way algorithm as the DataRobot CLI (last-synced state vs. local files vs. catalog) instead of a one-way upload. **Apply can now write to `source.dir`**: it keeps sync bookkeeping in `source.dir/.wapi/` (last-synced manifest, catalog pointers, lock file — gitignore it), downloads files that exist in the catalog but not locally, removes local files deleted from the catalog, and, for a file edited both locally and in the catalog, keeps the catalog version at the original path while preserving the local bytes as `<path>.LOCAL.<timestamp>` (`terraform apply` has no prompt, so the catalog wins). Only changed files are uploaded, and an unchanged tree whose catalog version has not moved makes no Files API calls at all. `.datarobot.yaml` is still never uploaded, and the default `.drignore` excludes `*.LOCAL.*` so conflict copies are not uploaded on the next apply. Directories already uploaded by an earlier provider version keep their existing catalog: the first sync seeds `.wapi/` from Terraform state instead of creating a second catalog. Note that, matching the CLI, a single sync touching more than 1000 files is refused rather than run without a usable rollback — add `.drignore` rules to bring a larger tree under the limit.
- `internal/artifactsource/sync` engine can now apply the remote half of a three-way sync plan and persist its result (`ExecuteRemote`): it removes catalog files the user deleted locally, uploads added and modified files through the existing stage/zip backend, patches the artifact's primary `code_ref` to the resulting catalog version, and writes the new BASE manifest under `source.dir/.wapi/`. A failed upload or delete restores `source.dir` to its pre-sync state; a failure while writing `.wapi/` does not roll the catalog back, since the next sync reconciles. Still not reachable from `datarobot_artifact`: wiring the engine into the resource lands in a follow-up change.
