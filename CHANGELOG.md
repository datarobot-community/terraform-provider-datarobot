...
- `use_case_id` still replaces the Workload, because the platform fixes the Use Case link at creation and offers no way to move it. Since the create after the destroy can be refused for the reasons above, set `lifecycle { create_before_destroy = true }` on a Workload whose Use Case may change.
