...
- **One `source.dir` per `datarobot_artifact` resource.** The sync state under a directory is bound to one catalog, so two resources sharing a `source.dir` now contend for its lock during a parallel apply (the loser fails naming the cause) and, once the directory is bound, the second is refused as pointing at a different catalog. Give each resource its own directory.
