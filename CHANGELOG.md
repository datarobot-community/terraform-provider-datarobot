...
- **One `source.dir` per `datarobot_artifact` resource.** The sync state under a directory is bound to one catalog, so a second resource over a directory that already backs another live artifact (one in a different artifact repository) is refused with the cause, and two resources racing for the same directory in a parallel apply fail on its lock the same way. Give each resource its own directory. A destroyed and re-created resource, and a locked artifact cloning to a new version, are not affected.
...
- `datarobot_artifact`: a source upload whose Files API response carries no catalog version is now an error that restores `source.dir`, instead of a sync that records the upload as done while the artifact keeps building from the previous version.
