...
- `datarobot_artifact`: a file deleted from `source.dir` is now removed from the catalog on the first sync in a directory that has no sync state yet. The sync compares three sides — the last-synced manifest (BASE), the local tree, and the catalog — and a deletion is the cell where BASE and the catalog have a path that the tree does not. A directory with no state is seeded with an *empty* BASE, and against an empty BASE a deleted file is in no side at all: it produced no row, so nothing was deleted, the whole tree re-uploaded as if every file were new, and the deleted file stayed in the new catalog version and kept reaching the image build. Edits and additions were unaffected, which is why only deletion looked broken. BASE is now read from the catalog version the directory is pinned at (`code_ref.catalog_version_id`, or what Terraform state records), which is what the tree was last pushed from and so is the real common ancestor. A directory whose artifact has no code anywhere yet still starts from an empty BASE and makes no extra API call.
- That empty BASE was the state of every first sync the state directory was missing for, not a rare edge case: `source.dir/.datarobot/workload/` carries its own `.gitignore` of `*`, so it is never committed, and it is absent on every fresh checkout, every CI runner, and every artifact created before 0.11.2. The same gap in a directory whose catalog had also moved on (the DataRobot CLI or another checkout pushed since) was worse than a stale catalog entry: the deleted file classified as a *remote addition* and apply downloaded it back into `source.dir`, undoing the deletion on disk as well.
- As a side effect of having a real ancestor, that first sync no longer re-uploads files it did not change; it uploads only what differs, the way every later sync already did. `pulumi up` lost deletions identically, since the bridge runs this same sync.
 a data source, or another resource. Terraform runs its validate walk before any of those resolve, so every non-literal value reaches `ValidateConfig` as *unknown*, and the resource's rules read unknown as "not set". A configuration that was in fact complete failed at plan time with `Missing image source` (`image_uri`), `Missing execution environment ID` / `Missing execution environment version ID` (`image_build_config.dockerfile`), `Missing source directory` (`source.dir`), `Invalid wait_for_build on locked artifact`, `Incomplete build configuration for locked artifact`, or `Unsupported on non-primary container` (`primary`). Unknown now means "set, not resolved yet"; only a null value counts as absent. This unblocks `examples/workflows/workload_replacement` (`image_uri = var.container_image`), which the workload docs link to as a runnable walkthrough, and lets the `datarobot_execution_environment` data source feed `execution_environment_id` and `execution_environment_version_id` instead of them being hardcoded. `pulumi preview` failed the same way, since the bridge validates the configuration with unresolved outputs; only `pulumi up --skip-preview` got through.
