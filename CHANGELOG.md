...
- `datarobot_workload` no longer destroys and recreates the Workload when `runtime.enclave_selection_policy` or `runtime.enclaves` changes. Both carried `RequiresReplace` since 0.12.2, so moving a Workload onto an Enclave, off it, or from the scheduler's choice to a pin planned a destroy of the running Workload and a create of a new one with a new ID and a new endpoint. The platform never required that: `PATCH /workloads/{id}/settings` and `POST /workloads/{id}/replacement` both accept a placement change and roll it out like any other runtime change, keeping the ID. The change now goes through the same in-place path as `runtime.container_groups`, on its own through the settings endpoint or together with an `artifact_id` change through the replacement endpoint. Whenever the placement changes, both `enclaveSelectionPolicy` and `enclaves` are written explicitly (a removed policy as `null`, a removed pin as `[]`), because on those endpoints omitting both fields means "keep the stored placement", which is the behaviour that lets a replica-only update leave an Enclave Workload where it is. The provider records the new intent; applying it is the platform's job, and one gap is known there: removing the policy from a Workload that already runs on an Enclave records no placement but does not move the Workload off its Enclave, it keeps serving from where it runs ([RAPTOR-20492](https://datarobot.atlassian.net/browse/RAPTOR-20492)). Plan and state then read as unplaced while the Workload is not. The endpoint is planned as `(known after apply)` when placement changes, since it is served from the Enclave the Workload runs on.
- The destroy-first order made the old behaviour worse than a new ID. Terraform destroys before it creates, and the create can be refused: on an organization with Enclaves the platform rejects a Use Case-linked Workload that names no placement policy, and a Use Case with no Enclave granted has no eligible Enclave. Reproduced on the HADR development cluster: removing `enclave_selection_policy` deleted the running Workload and the re-create failed with `422 USE_CASE_NOT_APPLICABLE`, leaving no Workload at all and the apply failed. The agent application template recomputes the policy from the organization's Enclave entitlement on every deploy, so there an entitlement toggle alone planned that destroy with no change to the program.
- `use_case_id` still replaces the Workload, because the platform fixes the Use Case link at creation and offers no way to move it. Since the create after the destroy can be refused for the reasons above, set `lifecycle { create_before_destroy = true }` on a Workload whose Use Case may change; workload names are not unique, so the two briefly coexist. Related asymmetry, on the platform side: removing `enclave_selection_policy` while keeping `use_case_id` now applies in place, but on an organization with Enclaves that configuration cannot be created from scratch (`ENCLAVE_TARGETING_REQUIRED`), so a destroy followed by an apply of the same configuration fails until the create and update paths agree.
...
- `datarobot_artifact` no longer accepts a configuration whose `image_uri` apply cannot honour. A container with `source`, `image_build_config` and an explicit `image_uri` planned cleanly and then failed the apply with `Provider produced inconsistent result after apply ... image_uri: was cty.StringVal(...), but now null`, after the artifact, its repository and the uploaded source already existed — and Terraform never writes a rejected result to state, so every retry left another artifact behind. The combination cannot work: a source-driven build produces the image and apply writes what the platform reports, which is the built image once the build finishes and nothing at all while it is still running, and Terraform will not let the provider plan its way out either, because a plan may not turn a known configured value unknown. It is now refused at plan time, before anything is created. `pulumi up` reported no error at all and recorded `imageUri: null` for a value the program set, which then showed as a one-resource diff on every subsequent preview; the bridge validates the same way, so it is refused there too.
- Locking an artifact without waiting for its build (`status = "locked"` with `source.wait_for_build = false`) is the one place an explicit `image_uri` is still required rather than refused, because the lock request needs an image the unfinished build has not produced. That rule and the new one are exact complements and a test walks the matrix to keep them that way, including the column where `wait_for_build` itself comes from a variable: that value is what picks between the two, so while it is unresolved neither fires and both wait for the plan walk. `examples/resources/datarobot_artifact` now shows that shape.
- A sidecar's `image_uri` no longer stops the primary container's from being planned as provider-managed. The check for a configured value scanned every container in the spec rather than the one it was deciding about, so a perfectly valid sidecar image pinned the primary to its stale value and apply wrote the built image over it — the same `inconsistent result after apply`, reached without configuring anything unusual.
...
- No check is lost by deferring. Terraform validates the configuration a second time during the plan walk, where variables and data sources have resolved, so `image_uri = var.image` with an empty `var.image` is still refused at plan. A reference to a resource created in the same apply is still unknown then, and Terraform validates once more on the apply walk, where that reference has resolved and the rules refuse it before the artifact is created. Acceptance tests could not catch any of this because their HCL is assembled with `fmt.Sprintf`, which yields a literal for every value; the new cases are written as configurations with real `variable` blocks.
