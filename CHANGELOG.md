...
- `datarobot_artifact` now surfaces artifact image build logs in the error message when a source upload triggers a build that fails (or when the build trigger fails). A link to the build log page in the DataRobot UI is included. Default tail length is 30 lines; override with `DATAROBOT_ARTIFACT_BUILD_LOGS_TAIL_LINES`.
