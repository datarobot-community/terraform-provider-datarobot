...
func workloadDataSourceTestCheckFuncs(dataSourceName string, w *client.Workload) []resource.TestCheckFunc {
	...
	checks := []resource.TestCheckFunc{
		...
		resource.TestCheckResourceAttr(dataSourceName, "runtime.container_groups.0.autoscaling.min_replica_count", fmt.Sprintf("%d", autoscaling.MinReplicaCount)),
		resource.TestCheckResourceAttr(dataSourceName, "runtime.container_groups.0.autoscaling.max_replica_count", fmt.Sprintf("%d", autoscaling.MaxReplicaCount)),
